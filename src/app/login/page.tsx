...
export default function LoginPage() {
  ...
  const { user, loading, signInWithGoogle, signInWithApple, signInWithWallet } = useAuth();
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl p-6 md:p-8">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center gap-4 px-5 py-3.5 rounded-xl bg-white text-dark-surface hover:bg-white/90 transition-all duration-300 hover:-translate-y-px hover:shadow-lg hover:shadow-white/10 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed group"
            >
              <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.05 20.28c-.98.95-2.05.88-3.08.4-1.09-.5-2.08-.48-3.24 0-1.44.62-2.2.44-3.06-.4C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.53 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.32 2.32-2.12 4.45-3.74 4.25z" />
              </svg>
              <span className="flex-1 text-left text-[14px] font-semibold">Continue with Apple</span>
              {authLoading === "apple" ? (
                <div className="w-5 h-5 border-2 border-dark-surface/20 border-t-dark-surface rounded-full animate-spin" />
              ) : (
